...
db/connector.py
---------------
DatabaseConnector — connects to SQLite, creates a sample DB if absent,
and exposes execute_query() with parameterized-query support.
"""
...
class DatabaseConnector:
        ...
        if db_type == "sqlite":
            ...
            if not os.path.exists(db_path):
                self.logger.log_system(
                    f"Database not found at {db_path} — creating sample database."
                )
                self._create_sample_database(db_path)

            self.connection = sqlite3.connect(db_path)
            self.connection.execute("PRAGMA query_only = ON")   # DB-level read-only
    ...
    def execute_query(
        ...
        """
        Execute *query* with optional *params* (for parameterized queries).
        Returns a result dict with keys: status, data, columns, error.
        """
        ...
        try:
            ...
            self.logger.log_system(
                f"Query executed: {len(data)} row(s) returned"
            ...
            self.logger.log_system(f"Query execution error: {exc}")
            return {
                "status":  "error",
                "error":   str(exc),
                "data":    [],
    ...
    # ── Sample database creation (unchanged from original) ────────────────────
